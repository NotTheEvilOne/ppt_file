...
class File(_PathLike):
    ...
    :author:     Tobias "NotTheEvilOne" Wolf et al.
    :copyright:  Tobias "NotTheEvilOne" Wolf - All rights reserved
    :package:    ppt
    :subpackage: file
    :since:      v1.0.0
...
umask to set before creating a new file
        ...
        if isinstance(default_chmod, int):
            ...
            if (1000 & default_chmod) == 1000:
                self.chmod |= stat.S_ISVTX
            if (2000 & default_chmod) == 2000:
                self.chmod |= stat.S_ISGID
            if (4000 & default_chmod) == 4000:
                self.chmod |= stat.S_ISUID
            if (0o100 & default_chmod) == 0o100:
                self.chmod |= stat.S_IXUSR
            if (0o200 & default_chmod) == 0o200:
                self.chmod |= stat.S_IWUSR
            if (0o400 & default_chmod) == 0o400:
                self.chmod |= stat.S_IRUSR
            if (0o010 & default_chmod) == 0o010:
                self.chmod |= stat.S_IXGRP
            if (0o020 & default_chmod) == 0o020:
                self.chmod |= stat.S_IWGRP
            if (0o040 & default_chmod) == 0o040:
                self.chmod |= stat.S_IRGRP
            if (0o001 & default_chmod) == 0o001:
                self.chmod |= stat.S_IXOTH
            if (0o002 & default_chmod) == 0o002:
                self.chmod |= stat.S_IWOTH
            if (0o004 & default_chmod) == 0o004:
                self.chmod |= stat.S_IROTH
    ...
    def __enter__(self):
        ...
        if self._handle is None:
            raise IOError("Failed to enter context for an uninitialized file instance")
    ...
    def is_eof(self):
        ...
        return (
            True
            if (self._handle is None or self._handle.tell() == self.file_size)
            else False
        )
    ...
    def log_handler(self, log_handler):
        ...
        self._log_handler = (
            log_handler if (isinstance(log_handler, ProxyTypes)) else proxy(log_handler)
        )
    ...
    def close(self, delete_empty=False):
        ...
        if self._log_handler is not None:
            self._log_handler.debug("ppt_file.File.close()")
    ...
    def _locking(self, lock_mode, file_path_name=""):
        ...
        if len(file_path_name) > 0 and self._handle is not None:
            if lock_mode == "w" and self.readonly:
                ...
                if lock_mode == "w":
                    if is_locked and self._handle_lock == "w":
                        _return = True
                    elif not is_locked:
                        try:
    ...
    def open(self, file_path_name, readonly=False, file_mode="r+b"):
        ...
        if self._handle is None:
            ...
            is_binary = True if ("b" in file_mode) else False
    ...
    def read(self, n=0, timeout=-1):
        ...
        if self.lock("r"):
            bytes_unread = n
            _return = bytes() if (self.binary) else ""
            timeout_time = None if (timeout < 0) else (time.time() + timeout)

            while (
                (bytes_unread > 0 or n == 0)
                and not self.is_eof
    ...
    def truncate(self, new_size=None):
        ...
        if self.lock("w"):
            ...
            raise IOError("Failed to truncate the file")
    ...
    def write(self, b, timeout=-1):
        ...
        if self.lock("w"):
            if self.binary and not isinstance(b, bytes):
                b = str.encode(b, "raw_unicode_escape")

            bytes_unwritten = len(b)
            bytes_written = self._handle.tell()

            if (bytes_written + bytes_unwritten) <= self.file_size:
                new_size = 0
            else:
                new_size = bytes_written + bytes_unwritten
